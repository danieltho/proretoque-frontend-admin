...
export default function QuotePage() {
  ...
  const handleDelete = async (id: number) => {
    await deleteQuoteAdminApi(id).send()
    send()
  }
  ...
  return (
    <Template>
      <div className="flex flex-col gap-4 font-raleway">
        <TitleSection breadcrumbs={[{ label: 'Pedidos' }]} title="Pedidos" />
        ...
        {loading ? (
          ...
          <p className="py-8 text-center text-muted-foreground">No hay pedidos registrados.</p>
        ) : (
